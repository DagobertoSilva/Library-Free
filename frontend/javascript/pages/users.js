...
import { FILTER_ALUNOS, GET_ALUNOS, DELETE_ALUNO } from '../services/api.js';
...
const userConfig = {
    ...
    renderRow: (user) => {
        const editIcon = `
        <button class="editIcon" data-user-id="${user.id}">
        ...
        `;
        const deleteIcon = `
        <button class="deleteIcon" data-user-id="${user.id}" data-user-name="${user.nome}">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    tableBody.addEventListener('click', (event) => {
        const editButton = event.target.closest('.editIcon');
        ...
        if (deleteButton) {
            ...
            const deleteUserAction = async () => {
                ...
                const { url, options } = DELETE_ALUNO(userId);
                const { error } = await fetchData(url, options);

                if (error) { 
                    alert(`Erro ao deletar o usuário "${userName}".`);
                } else { 
